...
DEFAULT_OUTPUT = Path("data/plots/word2vec_expansion_trend.png")
DEFAULT_CSV_OUTPUT = Path("data/plots/word2vec_expansion_trend.csv")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot how BM25 hit counts change as Word2Vec expansion top-k increases."
    )
    ...
    parser.add_argument("--max-topk", type=int, default=20, help="Maximum Word2Vec neighbors to include")
    ...
    parser.add_argument(
        "--plot-term-label",
        default=None,
        help="ASCII label shown in the plot title, for example: guichu",
    )
...
def get_expansion_terms(term: str, model_path: Path, max_topk: int, min_score: float) -> list[tuple[str, float | None]]:
    vectors = load_vectors(model_path)
    terms: list[tuple[str, float | None]] = [(term, None)]
    if term not in vectors:
        return terms

    neighbors = vectors.most_similar(term, topn=max_topk)
    for token, score in neighbors:
        if float(score) < min_score:
            continue
        terms.append((token, float(score)))
    return terms
...
def build_rows(args: argparse.Namespace) -> list[dict[str, object]]:
    expanded_terms = get_expansion_terms(args.term, args.model, args.max_topk, args.min_score)
    con = duckdb.connect(args.db.as_posix(), read_only=True)
    con.execute("LOAD fts")

    rows: list[dict[str, object]] = []
    baseline_query_terms = normalize_query_terms([args.term], args.tokenize_query)
    baseline_count = count_matches(
        con=con,
        table=args.table,
        query_terms=baseline_query_terms,
    ...
    for topk in range(len(expanded_terms)):
        current_terms = [token for token, _ in expanded_terms[: topk + 1]]
        normalized_terms = normalize_query_terms(current_terms, args.tokenize_query)
        hit_count = count_matches(
            con=con,
            table=args.table,
        ...
        added_term, added_score = expanded_terms[topk]
        rows.append(
            {
                "seed_term": args.term,
                "topk": topk,
                ...
                "delta_vs_baseline": hit_count - baseline_count,
            }
        )
    con.close()
    return rows
            ...
            writer.writerow(
                {
                    ...
                    "topk": row["topk"],
                    "added_term": row["added_term"],
                    "added_score": row["added_score"],
                    "hit_count": row["hit_count"],
                    "baseline_hit_count": row["baseline_hit_count"],
...
def plot_rows(rows: list[dict[str, object]], output: Path, dpi: int, plot_term_label: str | None) -> None:
    ...
    ys = [int(row["hit_count"]) for row in rows]
    baseline = int(rows[0]["baseline_hit_count"])
    term_label = plot_term_label or "seed-term"

    plt.figure(figsize=(10, 6))
    plt.plot(xs, ys, marker="o", linewidth=2, color="#0f766e", label="Word2Vec expansion")
    plt.axhline(
        baseline,
        color="#b91c1c",
        linestyle="--",
        linewidth=1.5,
        label=f"Direct search baseline = {baseline}",
    )
    plt.fill_between(xs, baseline, ys, color="#99f6e4", alpha=0.25)
    ...
    plt.xticks(xs)
    plt.xlabel("Expansion top-k")
    plt.ylabel("Matched video count")
    plt.title(f"Word2Vec expansion trend for '{term_label}'")
        ...
        json.dumps(
            {
                "term": args.term,
                "points": len(rows),
                "baseline_hit_count": rows[0]["baseline_hit_count"],
                "best_topk": max(rows, key=lambda row: int(row["hit_count"]))["topk"],
                "best_hit_count": max(rows, key=lambda row: int(row["hit_count"]))["hit_count"],
